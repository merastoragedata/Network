...
// Adopt positions from an uploaded .drawio file.
// positions = [{id, x, y, width}]
function applyDrawio(payload) {
  const sh = getSS().getSheetByName(SHEETS.SUB);
  const values = sh.getDataRange().getValues();
  const headers = values[0];
  const idC = headers.indexOf('id');
  const xC = headers.indexOf('x'), yC = headers.indexOf('y'), wC = headers.indexOf('busWidth');
  const index = {};
